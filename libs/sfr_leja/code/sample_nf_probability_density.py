import torch
from sfr_leja.code import torch_light
import numpy as np
...
def sample_density(flow, 
                   dens_params={},
                   redshift_smoothing=True):
    ...
    ### to compute the probability density
    ### the spacing of this grid can be altered as desired
    nlogm, nsfr, dz = dens_params["nlogm"], dens_params["nsfr"], dens_params["dz"]
    ndummy = dens_params["ndummy"]
    mmin, mmax = dens_params["mmin"], dens_params["mmax"]
    sfrmin, sfrmax = dens_params["sfrmin"], dens_params["sfrmax"]
    zmin, zmax = dens_params["zmin"], dens_params["zmax"]

    print("The star formation density is sampled using the following parameters:")
    print(f"nlogm={nlogm}, nsfr={nsfr}, dz={dz}")
    print(f"ndummy={ndummy}")
    print(f"mmin={mmin}, mmax={mmax}")
    print(f"sfrmin={sfrmin}, sfrmax={sfrmax}")
    print(f"zmin={zmin}, zmax={zmax}")
    print('Sampling the posterior has started')
