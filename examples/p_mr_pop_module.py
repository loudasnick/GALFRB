...
def load_p_mr_distributions(fname: str = 'p_mr_distributions_dz0.01_z_in_0_2.0.h5', output_dir: str = 'p_mr_dists/') -> tuple:
    """
    ...
    Returns:
        ...
        p_mr_unweighted (np.array): p(mr|z) for unweighted population. Shape: (len(zbins) - 1, rmag_resolution). rmag_resolution(=len(rmag_centers)) is fixed across redshift bins.
    Note:
    ...
    def give_p_mr_unweighted(z: float):
        """
        Function to return p(mr|z) for unweighted population.
        Args:
            z (float): Redshift value.
        Returns:
            np.array: p(mr|z) values.
        Note:
            This function assumes that the redshift bins are defined in the `unweighted_population` data.
            Given the fine discretization of redshift bins, it uses the nearest bin for the provided redshift value.
            rmag_centers and p_mr_unweighted are defined in the outer scope of this function.
        """
        # Find the appropriate redshift bin index
        idx = np.clip(np.searchsorted(zbins, z) - 1, 0,  n_redshift_bins - 1)
        return p_mr_unweighted[idx]

    return zbins, rmag_centers, p_mr_sfr, p_mr_mass, p_mr_unweighted, give_p_mr_sfr, give_p_mr_mass, give_p_mr_unweighted

zbins, rmag_centers, p_mr_sfr, p_mr_mass, p_mr_unweighted, give_p_mr_sfr, give_p_mr_mass, give_p_mr_unweighted = load_p_mr_distributions(fname = 'p_mr_distributions_dz0.01_z_in_0_2.0.h5', output_dir = 'p_mr_dists/')